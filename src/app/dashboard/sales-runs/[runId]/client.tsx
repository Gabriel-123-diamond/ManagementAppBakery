...
export default function SalesRunDetailsPageClientContent({ runId }: { runId: string }) {
    ...
    if (!runId) {
        return (
            <div className="flex flex-col items-center justify-center h-full text-center">
                ...
                <Button asChild className="mt-4"><Link href="/dashboard/deliveries">Go to Deliveries</Link></Button>
            </div>
        );
    }
    ...
    const runComplete = runStatus === 'completed' || runStatus === 'return_completed';
    const isPendingReturn = runStatus === 'pending_return';
    const isReadOnly = user?.role === 'Manager' || user?.role === 'Supervisor' || user?.role === 'Accountant';
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
                <Card>
                    ...
                    <CardContent className="space-y-2">
                        ...
                        <div><Progress value={(totalCollected / run.totalRevenue) * 100} /></div>
                        <div className="text-sm text-muted-foreground">Collection Progress ({formatCurrency(totalCollected)} / {formatCurrency(run.totalRevenue)})</div>
                    </CardContent>
                </Card>
                ...
                <TabsContent value="pending">
                    <Card>
                        <CardHeader>
                            ...
                            <CardDescription>All items awaiting confirmation by an accountant.</CardDescription>
                        </CardHeader>
                        <CardContent>
                            <Table>
                                <TableHeader><TableRow><TableHead>Date</TableHead><TableHead>Type</TableHead><TableHead>Details</TableHead><TableHead>Status</TableHead><TableHead className="text-right">Amount</TableHead></TableRow></TableHeader>
                                <TableBody>
                                    {paymentConfirmations.length > 0 ? paymentConfirmations.map(p => (
                                        <TableRow key={p.id}>
                                            <TableCell>{format(new Date(p.date.toDate()), 'Pp')}</TableCell>
                                            <TableCell><Badge variant="outline">{p.type}</Badge></TableCell>
                                            <TableCell className="text-sm">
                                                {p.customerName && `Customer: ${p.customerName}`}<br/>
                                                {p.items && `${p.items.reduce((s:number,i:any)=>s+i.quantity,0)} items`}
                                            </TableCell>
                                       ...
                                       <TableRow>
                                            <TableCell colSpan={5} className="h-24 text-center text-muted-foreground">
                                                No pending confirmations for this run.
                                            </TableCell>
                                        </TableRow>
                                    )}
                                </TableBody>
            ...
            {viewingOrder && (
                <Dialog open={!!viewingOrder} onOpenChange={() => setViewingOrder(null)}>
                     <DialogContent>
                        ...
                        <Receipt order={viewingOrder} ref={summaryReceiptRef} />
                        <DialogFooter className="gap-2 sm:justify-end">
                            <Button variant="outline" onClick={() => setViewingOrder(null)}>Close</Button>
                            <Button onClick={() => handlePrint(summaryReceiptRef.current)}><Printer className="mr-2 h-4 w-4" />Print</Button>
                        </DialogFooter>
                    </DialogContent>
                </Dialog>
            )}
